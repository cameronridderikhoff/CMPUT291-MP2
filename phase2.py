# Given the sorted files terms.txt, emails.txt, dates.txt and recs.txt, create
...
# the character strings before colon ':' and the data is everything that comes after
# the colon.
...
# The mv command is a command similar to cp, but it is not a copy of a file or
# directory.
# remove old index
# If the option -T is specified, then the suboption -t;
# suboption -t must be appended, appended to the database type used to specify
# translation loading after the -T option.
# The -f parameter is followed by a text file containing the username and
# password. The contents of the file are: odd row username, even row password.
